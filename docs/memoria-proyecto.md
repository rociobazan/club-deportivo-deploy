...
24. **Lo que catálogo y disponibilidad dejan fijado** (2026-09-26; cambio OpenSpec `implementar-catalogo-y-disponibilidad`, ítem 1.2, PR apilado sobre el #19).
    - **`Reloj`** (`common/reloj.ts`, global): `ahora()` da `{ fecha, hora }` en `ZONA_HORARIA_CLUB` con `Intl`, sin librerías. **Todo lo que compara contra el presente pasa por ahí** (1.3 y 1.4: RN-02, RN-03, RN-04, RN-07, `COMPLETADA`); los e2e lo reemplazan con `overrideProvider(Reloj)`.
    - **`generarGrilla(apertura, cierre, duracionMin)`** (`common/grilla.ts`) es una función pura anclada a la apertura; `HORA_APERTURA`, `HORA_CIERRE` y `ZONA_HORARIA_CLUB` tienen valor por defecto y se validan al arrancar. El front tiene **una copia** (`lib/grilla.ts`, `lib/club.ts`) porque el contrato devuelve solo los turnos libres; es duplicación consciente.
    - **Los precios salen como `number`**: `Prisma.Decimal` serializa como string. Los mapeadores de `catalogo/mapeadores.ts` (`aCancha`, `aEquipamiento`, `aNumero`) son el único lugar donde se convierte; reservas y administración los reutilizan. `aFechaDb`/`deFechaDb` son el único puente entre `YYYY-MM-DD` y la columna `@db.Date`.
    - **El guard de JWT identifica al usuario también en endpoints públicos**: si viene un token válido queda en `request.usuario`; si no, sigue como anónimo. `exigirRol(solicitud, 'ADMIN')` resuelve 401/403 en los públicos con variante privada (`incluirInactivas`, `incluirInactivos`). 1.6 lo hereda.
    - **Los e2e corren en serie** (`maxWorkers: 1` en `jest-e2e.json`): comparten una base y las limpiezas de un archivo pisaban los datos de otro. El CI no corre el seed, así que cada e2e crea sus datos (`e2e …`, `E2E-…`) y borra solo esos.
    - **Front**: `/disponibilidad` consulta por URL con el `<Form>` de Next (GET, sin estado en el cliente); el aviso para visitantes es un único `<dialog>` por página con clic delegado sobre `data-turno-libre`; con sesión, los bloques libres son links a `/reservar?canchaId=&fecha=&horaInicio=` (1.3). Un fallo de la API se muestra con `EstadoError` y un link de reintento.
...
- **Base visual del front (decisión 18)**: PR #10, mergeado. Tokens, tipografías, las cuatro primitivas y la página `/estilos`. `apps/web/app/page.tsx` sigue siendo la home por defecto de Next: la reemplaza la landing (1.5).
- **Tipos del contrato (decisión 19)**: PR #11, mergeado. Script `generate:api-types`, `schema.d.ts` versionado y los alias de `lib/api/types.ts`. Lint y build del workspace `web` en verde.
...
- **2026-09-26**: se implementó el ítem **1.2** (RF-01, RF-02, RF-03 y RF-07): módulos `catalogo` y `disponibilidad`, `Reloj`, grilla, precios como número (decisión 24) y las páginas `/canchas` y `/disponibilidad`. El PR sale apilado sobre el #19, que sigue esperando aprobación; se retargetea a `main` cuando entre. 98 unitarios y 65 e2e en verde; los 11 escenarios de pantalla verificados a mano. Al probar se vio que los e2e corrían en paralelo contra la misma base y se pisaban: ahora corren en serie.
